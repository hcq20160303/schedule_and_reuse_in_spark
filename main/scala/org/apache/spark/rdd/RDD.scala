...
abstract class RDD[T: ClassTag](
  ...
  def saveAsObjectFile(path: String): Unit = withScope {
    val mapRDD = this.mapPartitions(iter => iter.grouped(10).map(_.toArray))
   ...
   * ----------- hcq add code -------------
   */
  var indexOfnodesList: Int = -1         // use to find the coordinate SimulateRDD
  var indexOfleafInNodesList: Int = -1   // use to find the first leaf node in a DAG-Tree which root is this RDD
  var transformation: String = "default"
  var function: String = "default"
  var isCache = false       // use to judge a job which the finalRDD is this RDD whether generated by RDDShare
  var fromCache = false     // use to check if a rdd read data from repository

  def changeDependeces( parent: RDD[_]): Unit = {
    this.dependencies_ = List(new OneToOneDependency(parent))
  }

  def getElementClassTag = elementClassTag
}
